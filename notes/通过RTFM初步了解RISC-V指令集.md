...
`LW`（Load Word）使用 **I-type 格式**，`SW`（Store Word）使用 **S-type 格式**（手册 2.1.4.3 节 "Load and Store Instructions"）：
...
> "LW loads a 32-bit value from memory into register rd. SW stores a 32-bit value from register rs2 to memory."

- `LW`：`R[rd] = M[R[rs1] + sign_extend(imm)]` —— 从内存读 32 位
- `SW`：`M[R[rs1] + sign_extend(imm)] = R[rs2]` —— 向内存写 32 位

**S-type 立即数拆分原因**：S-type 和 B-type 将立即数拆成两段放在不同位置，目的是让 rs1、rs2、rd 字段在所有格式中保持相同位位置，简化解码器硬件。

**举例**：

| 指令 | 机器码 | 说明 |
|------|--------|------|
| `lw x5, 4(x2)` | `0x00412283` | 从 sp+4 加载 32 位 → x5 |
| `sw x5, 8(x2)` | `0x00512423` | 将 x5 存入 sp+8 |
