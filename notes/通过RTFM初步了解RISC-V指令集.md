...
`LUI` 使用 **U-type 格式**（手册第 2.1.4.5 节 "Upper Immediate Instructions"）：

```
 31                            12 | 11    7 | 6       0
┌────────────────────────────────┬─────────┬──────────┐
│          imm[31:12]            │    rd   │  opcode  │
│           20 bits              │  5 bits │ 0110111  │
└────────────────────────────────┴─────────┴──────────┘
```

- **opcode** = `0110111`
- **rd**（位[11:7]）：目标寄存器
- **imm[31:12]**（位[31:12]）：20 位立即数，左移 12 位后低 12 位填 0
- **无 rs1 / rs2 / funct3 / funct7**：U-type 不需要读寄存器，一条 opcode 对应唯一指令

**功能**：将 20 位立即数加载到寄存器的高 20 位，低 12 位清零（Load Upper Immediate）。
...
**举例**：`lui x5, 0x12345` → `x5 = 0x12345000`

**典型用途**：与 ADDI 配合构造 32 位立即数：
```
...
| 特征 | R-type (ADD) | I-type (ADDI) | U-type (LUI) |
|------|-------------|---------------|--------------|
| opcode | `0110011` | `0010011` | `0110111` |
| 读寄存器 | rs1 + rs2 | rs1 | 无 |
| 立即数 | 无 | 12 bits | 20 bits |
| 用途 | 寄存器-寄存器运算 | 寄存器-立即数运算 | 加载高位立即数 |

### `lw` 指令的格式具体是什么？

`LW`（Load Word）使用 **I-type 格式**（手册 2.1.4.3 节）：

```
 31         20 | 19    15 | 14  12 | 11    7 | 6       0
┌──────────────┬──────────┬────────┬──────────┬──────────┐
│  imm[11:0]   │   rs1    │ funct3 │    rd    │  opcode  │
│  12 bits     │  5 bits  │  010   │  5 bits  │ 0000011  │
└──────────────┴──────────┴────────┴──────────┴──────────┘
```
...
> "LW loads a 32-bit value from memory into register rd."

语义：`R[rd] = M[R[rs1] + sign_extend(imm)]`

**举例**：`lw x5, 4(x2)` → `x5 = M[sp + 4]`（机器码 `0x00412283`）

### `sw` 指令的格式具体是什么？

`SW`（Store Word）使用 **S-type 格式**（手册 2.1.4.3 节）：

```
 31       25 | 24   20 | 19    15 | 14  12 | 11      7 | 6       0
┌────────────┬─────────┬──────────┬────────┬───────────┬──────────┐
│  imm[11:5] │   rs2   │   rs1    │ funct3 │ imm[4:0]  │  opcode  │
│   7 bits   │  5 bits │  5 bits  │  010   │  5 bits   │ 0100011  │
└────────────┴─────────┴──────────┴────────┴───────────┴──────────┘
```
...
- **funct3** = `010`（SW）
- **rs1**（位[19:15]）：基地址寄存器
- **rs2**（位[24:20]）：要存储的源寄存器
- **imm[11:5]**（位[31:25]）+ **imm[4:0]**（位[11:7]）：12 位有符号偏移量（拆成两段）

> S-type 将立即数拆分是为了让 rs1、rs2 字段在所有格式中保持相同位位置，简化解码器硬件。

**功能**：将寄存器的 32 位值存入内存。

> "SW stores a 32-bit value from register rs2 to memory."

语义：`M[R[rs1] + sign_extend(imm)] = R[rs2]`

**举例**：`sw x5, 8(x2)` → `M[sp + 8] = x5`（机器码 `0x00512423`）

### `lbu` 指令的格式具体是什么？

`LBU`（Load Byte Unsigned）使用 **I-type 格式**（手册 2.1.4.3 节）：

```
 31         20 | 19    15 | 14  12 | 11    7 | 6       0
┌──────────────┬──────────┬────────┬──────────┬──────────┐
│  imm[11:0]   │   rs1    │ funct3 │    rd    │  opcode  │
│  12 bits     │  5 bits  │  100   │  5 bits  │ 0000011  │
└──────────────┴──────────┴────────┴──────────┴──────────┘
```

- **opcode** = `0000011`（LOAD，与 LW 相同）
- **funct3** = `100`（LBU）
- **rs1**（位[19:15]）：基地址寄存器
- **rd**（位[11:7]）：目标寄存器
- **imm[11:0]**（位[31:20]）：12 位有符号偏移量

**功能**：从内存加载 1 字节（8 位），**零扩展**到 32 位后写入 rd。

> "LBU loads a byte from memory, zero-extends it to XLEN bits, and writes it to rd."

语义：`R[rd] = zero_ext(M[R[rs1] + sign_extend(imm)][7:0])`

**与 LB（有符号版本）的对比**：

| 指令 | funct3 | 扩展方式 | 说明 |
|------|--------|----------|------|
| LB | `000` | 符号扩展 | `0xFF` → `0xFFFFFFFF` |
| LBU | `100` | 零扩展 | `0xFF` → `0x000000FF` |

**举例**：`lbu x5, 0(x2)` → 从 sp+0 读 1 字节，零扩展 → x5

### `sb` 指令的格式具体是什么？

`SB`（Store Byte）使用 **S-type 格式**（手册 2.1.4.3 节）：

```
 31       25 | 24   20 | 19    15 | 14  12 | 11      7 | 6       0
┌────────────┬─────────┬──────────┬────────┬───────────┬──────────┐
│  imm[11:5] │   rs2   │   rs1    │ funct3 │ imm[4:0]  │  opcode  │
│   7 bits   │  5 bits │  5 bits  │  000   │  5 bits   │ 0100011  │
└────────────┴─────────┴──────────┴────────┴───────────┴──────────┘
```

- **opcode** = `0100011`（STORE，与 SW 相同）
- **funct3** = `000`（SB）
- **rs1**（位[19:15]）：基地址寄存器
- **rs2**（位[24:20]）：源寄存器（取其低 8 位写入内存）
- **imm[11:5]**（位[31:25]）+ **imm[4:0]**（位[11:7]）：12 位有符号偏移量

**功能**：将 rs2 的低 8 位（1 字节）写入内存。

> "SB stores the low byte of register rs2 to memory."

语义：`M[R[rs1] + sign_extend(imm)] = R[rs2][7:0]`

**举例**：`sb x5, 0(x2)` → 将 x5 低 8 位 → M[sp+0]

**四条 Load/Store 指令汇总**：

| 指令 | 格式 | opcode | funct3 | 数据宽度 | 扩展方式 |
|------|------|--------|--------|----------|----------|
| LW | I-type | `0000011` | `010` | 32 bits | — |
| LBU | I-type | `0000011` | `100` | 8 bits | 零扩展 |
| SW | S-type | `0100011` | `010` | 32 bits | — |
| SB | S-type | `0100011` | `000` | 8 bits | — |
